...
import type { AchievementRarity } from './api/types/settings';
...
// --- Mock-specific types ---
// These represent the shapes used internally by mock data.
// They intentionally differ from strict API types (e.g., string IDs).
...
interface MockAchievementDefinition {
...
interface MockDaylioJob {
    ...
    errors: Array<{ index: number | null; reason: string }>;
    created_at: string;
    started_at: string | null;
    finished_at: string | null;
}

interface MockGalleryPhoto {
    id: string;
    entry_id: string;
    file_path: string;
    file_type: string;
...
const defaultMockEntries: MockEntry[] = [
    // Today
    {
        id: 'mock-entry-1',
    ...
    // 20 days ago
    {
        ...
        user_id: 'mock-user-001',
        mood: 5,
        note: "Got promoted at work!! \u{1F389} All that hard work paid off. Celebrated with champagne with the team.",
    ...
    // 32 days ago
    {
        ...
        user_id: 'mock-user-001',
        mood: 5,
        note: "Beach day! \u{1F3D6}\u{FE0F} Perfect weather, crystal clear water. Built sandcastles like a kid. So refreshing.",
...
const defaultMockGroups: MockGroup[] = [
    {
        id: 'group-1',
        name: 'Exercise',
        icon: '\u{1F4AA}',
...
const defaultMockGoals: MockGoal[] = [
    {
        id: 'goal-1',
        user_id: 'mock-user-001',
...
export const mockStatistics = {
    total_entries: 32,
    average_mood: 3.7,
    mood_distribution: { 1: 1, 2: 5, 3: 8, 4: 12, 5: 6 } as Record<number, number>,
    entries_this_week: 7,
...
const mockAchievementDefinitions: MockAchievementDefinition[] = [
...
const mockDaylioJobs = new Map<string, MockDaylioJob>();
...
export const mockApiService = {
    ...
    async getMoodEntries() {
        await delay(300);
        const entries = getStoredData<MockEntry>(STORAGE_KEYS.ENTRIES, defaultMockEntries);
    ...
    async createMoodEntry(entryData: Partial<MockEntry>) {
        await delay(300);
        const entries = getStoredData<MockEntry>(STORAGE_KEYS.ENTRIES, defaultMockEntries);
        ...
        return { entry: newEntry, new_achievements: [] as unknown[] };
    },

    async updateMoodEntry(entryId: string, entryData: Partial<MockEntry>) {
        await delay(300);
        const entries = getStoredData<MockEntry>(STORAGE_KEYS.ENTRIES, defaultMockEntries);
    ...
    async getEntrySelections(entryId: string) {
        await delay(100);
        const entries = getStoredData<MockEntry>(STORAGE_KEYS.ENTRIES, defaultMockEntries);
    ...
    async getEntryMedia(entryId: string) {
        await delay(100);
        const entries = getStoredData<MockEntry>(STORAGE_KEYS.ENTRIES, defaultMockEntries);
        const entry = entries.find(e => e.id === entryId);
        return entry?.media || [];
    ...
    async getStatistics() {
        ...
        return {
            ...
            average_mood: entries.length ? Number((total / entries.length).toFixed(1)) : 0,
            ...
            most_common_mood: Number(Object.entries(moodDist).sort((a, b) => b[1] - a[1])[0]?.[0] || 3),
    ...
    async getStreakDetails() {
        await delay(100);
        return { ...mockStreak, streak_dates: [] as string[] };
    },

    // Groups
    async getGroups() {
        await delay(200);
        return getStoredData<MockGroup>(STORAGE_KEYS.GROUPS, defaultMockGroups);
    },

    async createGroup(groupData: { name: string; icon?: string } | string) {
        await delay(300);
        const groups = getStoredData<MockGroup>(STORAGE_KEYS.GROUPS, defaultMockGroups);
        const name = typeof groupData === 'string' ? groupData : groupData.name;
        const icon = typeof groupData === 'string' ? '\u{1F4C1}' : (groupData.icon || '\u{1F4C1}');
    ...
    async createGroupOption(groupId: string, optionData: { name: string; icon?: string } | string) {
        await delay(200);
        const groups = getStoredData<MockGroup>(STORAGE_KEYS.GROUPS, defaultMockGroups);
        const group = groups.find(g => g.id === groupId);
        if (group) {
            const newOption: MockGroupOption = {
                id: `opt-${Date.now()}`,
                name: typeof optionData === 'string' ? optionData : optionData.name,
                group_id: groupId,
            };
    ...
    async deleteGroupOption(optionId: string) {
        await delay(200);
        const groups = getStoredData<MockGroup>(STORAGE_KEYS.GROUPS, defaultMockGroups);
        groups.forEach(g => {
    ...
    async moveGroupOption(optionId: string, newGroupId: string) {
        await delay(200);
        const groups = getStoredData<MockGroup>(STORAGE_KEYS.GROUPS, defaultMockGroups);
        let option: MockGroupOption | null = null;
        groups.forEach(g => {
        ...
        if (option) {
            ...
            if (targetGroup) {
                (option as MockGroupOption).group_id = newGroupId;
    ...
    async getGoals() {
        await delay(200);
        return getStoredData<MockGoal>(STORAGE_KEYS.GOALS, defaultMockGoals);
    },

    async createGoal(goalData: Partial<MockGoal>) {
        await delay(300);
        const goals = getStoredData<MockGoal>(STORAGE_KEYS.GOALS, defaultMockGoals);
        const newGoal: MockGoal = {
            id: `goal-${Date.now()}`,
            user_id: 'mock-user-001',
            title: goalData.title ?? '',
            description: goalData.description ?? '',
            frequency_per_week: goalData.frequency_per_week ?? 1,
            frequency_type: goalData.frequency_type ?? 'weekly',
    ...
    async updateGoal(goalId: string, patch: Partial<MockGoal>) {
        await delay(200);
        const goals = getStoredData<MockGoal>(STORAGE_KEYS.GOALS, defaultMockGoals);
    ...
    async incrementGoalProgress(goalId: string) {
        await delay(200);
        const goals = getStoredData<MockGoal>(STORAGE_KEYS.GOALS, defaultMockGoals);
    ...
    async checkAchievements() {
        await delay(200);
        return { new_achievements: [] as unknown[] };
    },
    ...
    async getAnalyticsCoOccurrenceByMood(mood: number | string) {
        await delay(200);

        const highMood = [
            { option1_name: 'Exercise', option2_name: 'Nature', count: 6, average_mood: 4.8, option1_icon: '\u{1F4AA}', option2_icon: '\u{1F3AF}' },
            { option1_name: 'Friends', option2_name: 'Food', count: 5, average_mood: 4.7, option1_icon: '\u{1F465}', option2_icon: '\u{1F37D}\u{FE0F}' },
            { option1_name: 'Nature', option2_name: 'Photo', count: 5, average_mood: 4.6, option1_icon: '\u{1F3AF}', option2_icon: '\u{1F4F8}' },
        ];

        const lowMood = [
            { option1_name: 'Poor Sleep', option2_name: 'Stress', count: 3, average_mood: 2.1, option1_icon: '\u{2764}\u{FE0F}', option2_icon: '\u{1F4AD}' },
            { option1_name: 'Stress', option2_name: 'Work', count: 3, average_mood: 2.3, option1_icon: '\u{1F4AD}', option2_icon: '\u{1F3AF}' },
            { option1_name: 'Sick', option2_name: 'Tired', count: 2, average_mood: 1.8, option1_icon: '\u{2764}\u{FE0F}', option2_icon: '\u{2764}\u{FE0F}' },
        ];
    ...
    async getAnalyticsBatch() {
        ...
        const scaleEntries: Array<{ date: string; name: string; value: number }> = [];
        for (let i = 6; i >= 0; i--) {
            const date = new Date();
    ...
    async getMoodDefinitions() {
        await delay(100);
        return [
            { score: 1, label: 'Awful', color: '#ef4444', emoji: '\u{1F622}' },
            { score: 2, label: 'Bad', color: '#f97316', emoji: '\u{1F615}' },
            { score: 3, label: 'Okay', color: '#eab308', emoji: '\u{1F610}' },
            { score: 4, label: 'Good', color: '#84cc16', emoji: '\u{1F642}' },
            { score: 5, label: 'Great', color: '#22c55e', emoji: '\u{1F604}' },
        ];
    },

    async updateMoodDefinition(score: number, updates: Record<string, unknown>) {
        await delay(200);
    ...
    async getReminders() {
        await delay(120);
        return getStoredData<MockReminder>(STORAGE_KEYS.REMINDERS, []);
    },

    async createReminder(payload: { time: string; days?: number[]; message?: string; goal_id?: number | null; is_active?: boolean }) {
        await delay(180);
        const reminders = getStoredData<MockReminder>(STORAGE_KEYS.REMINDERS, []);
    ...
    async updateReminder(reminderId: number, payload: Partial<MockReminder>) {
        await delay(180);
        const reminders = getStoredData<MockReminder>(STORAGE_KEYS.REMINDERS, []);
    ...
    async importDaylioBackup(_file: unknown, dryRun = false) {
        await delay(220);
    ...
    async getDaylioImportJob(jobId: string): Promise<MockDaylioJob> {
        await delay(140);
        return (
    ...
    // Gallery
    async getGalleryPhotos({ limit = 50, offset = 0, startDate, endDate }: { limit?: number; offset?: number; startDate?: string; endDate?: string } = {}) {
        await delay(200);
        const entries = getStoredData<MockEntry>(STORAGE_KEYS.ENTRIES, defaultMockEntries);
        ...
        const photos: MockGalleryPhoto[] = entries.flatMap((entry) => {
